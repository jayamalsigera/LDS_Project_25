...
consensusSteps = 3; % Iterations for DSEACP consensus
% consensusSteps = 50;
...
% KL-divergence tolerance for robust filters
rdkfb=0.05;



%% Network Definition
%Creating the graph describing the sensor network
...
%% Model Simulation
%Creating the plant Wiener model
plant = SingleTarget2dModel(Ts, sensorCount, outputNoiseStd, T);
% Generates A, B, C, D matrices and simulates noisy measurements
...
%Matrix for calculating error norm for stochastic trigger
srdkfZ=30*eye(plant.n);
...
ckf = CKF(plant, Ts, T); %Central Kalman Filter
dseacp = DSEACP(plant, Ts, T, netGraph, consensusSteps); %Distributed Kalman Filter with consensus steps
dkf = DKF(plant, Ts, T, netGraph, dkfAlpha, dkfBeta, dkfDelta); %Distributed kalman filter with event-triggered communication
rdkf=RDKF(plant, Ts, T, netGraph, dkfAlpha, dkfBeta, dkfDelta,rdkfb); %Robust distributed kalman filter with event-triggered communication
srdkf=SRDKF(plant, Ts, T, netGraph, dkfAlpha, dkfBeta, dkfDelta,rdkfb,srdkfZ); %Robust distributed kalman filter with event-triggered communication with stochastic trigger condition
